...
vector<pixel_t> MyALEScreen:: initial_background_image_;
size_t MyALEScreen:: num_initial_background_pixels_;
...
void run_episode(ALEInterface &env,
    ...
    for( size_t frame = 0; !env.game_over() && (frame < max_execution_length_in_frames); frame += frameskip ) {
        ...
        if( branch.empty() ) {
            ...
            if( branch.empty() ) {
               logging::Logger::Error << "no more available actions!" << endl;
                break;
            }
            ...
            logging::Logger::Info << "executable-prefix: len=" << branch.size() << ", actions=[";
            for( size_t j = 0; j < branch.size(); ++j )
                logging::Logger::Continuation(logging::Logger::Info) << branch[j] << ",";
            logging::Logger::Continuation(logging::Logger::Info) << "]" << endl;
        }
       ...
       logging:: Logger::Stats << "step-stats: acc-reward=" << g_acc_reward << ", acc-frames=" << g_acc_frames << endl;
...
int main(int argc, char **argv) {
    ...
    logging::Logger::set_output_stream(*default_log_file);
    logging::Logger::set_mode(logging::Logger::Info);
    logging::Logger::set_use_color(true);
    logging::Logger::set_debug_threshold(0);
    ...
    opt_desc.add_options()
      // rom and log files
      ("logging::logger-mode", po::value<string>(&opt_logger_mode)->default_value("info"), "Change logging::logger mode to 'debug', 'info', 'warning', 'error', 'stats', or 'silent' (default is 'info')")
      ...
      ("ale-logging::logger-mode", po::value<string>(&opt_ale_logger_mode)->default_value("error"), "Change ALE logging::logger mode to 'info', 'warning', 'error', or 'silent' (default is 'error')")
      ...
      // planners
      ("planner", po::value<string>(&opt_planner_str)->default_value(string("bfs")), "Set planner, either 'rollout' or 'bfs' or 'iw' (default is 'rollout')")
      ...
      ("lookahead_depth", po::value<int>(&lookahead_depth)->default_value(lookahead_depth), "How far to look ahead in the tree (default is 5)")
      // options for rollout planner
      ("max-depth", po::value<int>(&opt_max_depth)->default_value(1500), "Set max depth for lookahead (default is 1500)")
        // options for iw planner
       ("width", po::value<int>(&opt_width)->default_value(1), "Set width for IW planner (default is 1)")  
       ("printing-sketches", po::value<int>(&opt_printing_sketches)->default_value(0),"Print sketches of IW planner (default is off)")
       // options for bfs planner
    ...
    try {
        ...
        logging::Logger::Error << e.what() << endl;
        exit(1);
    }
    ...
    // set logging::logger mode and log mode
    logging::Logger::mode_t logger_mode = logging::Logger::Silent;
    if( opt_logger_mode == "debug" ) {
        logger_mode = logging::Logger::Debug;
    } else if( opt_logger_mode == "info" ) {
        logger_mode = logging::Logger::Info;
    } else if( opt_logger_mode == "warning" ) {
       logger_mode = logging::Logger::Warning;
    } else if( opt_logger_mode == "error" ) {
        logger_mode = logging::Logger::Error;
    } else if( opt_logger_mode == "stats" ) {
        logger_mode = logging::Logger::Stats;
    } else if( opt_logger_mode == "silent" ) {
        logger_mode = logging::Logger::Silent;
    } else {
        logging::Logger::Error << "invalid logging::logger mode '" << opt_logger_mode << "'" << endl;
        exit(-1);
    }
    logging::Logger::set_mode(logger_mode);
    logging::Logger::set_debug_threshold(opt_debug_threshold);

    if( opt_varmap.count("log-file") && (opt_log_file != "") ) {
        ostream *logger_output_stream = new ofstream(opt_log_file);
        logging::Logger::set_output_stream(*logger_output_stream);
        logging::Logger::set_use_color(false);
    }
    ...
    print_options(logging::Logger::output_stream(), opt_varmap);

    // set random seed for lrand48() and drand48()
    srand48(opt_random_seed);

    // set logging::logger mode for ALE
   ale::Logger::mode ale_loggine = ale::Logger::Info;
    if( opt_ale_logger_mode == "info" ) {
        ale_loggine = ale::Logger::Info;
    } else if( opt_ale_logger_mode == "warning" ) {
        ale_loggine = ale::Logger::Warning;
    } else if( opt_ale_logger_mode == "error" ) {
        ale_loggine = ale::Logger::Error;
    } else {
        logging::Logger::Error << "invalid ALE logging::logger mode '" << opt_ale_logger_mode << "'" << endl;
        exit(-1);
    }
    ...
    if( opt_screen_features > 0 ) {
        ...
        MyALEScreen::compute_breakout_background(sim, opt_frames_for_background_image);
    ...
    if( opt_fixed_action_sequence != "none" ) {
        ...
        if( opt_planner_str == "rollout" ) {
            ...
            planner = new RolloutIW(sim,
                                    ...
                                    opt_nodes_threshold,
                                    opt_max_depth, lookahead_depth, sketches);
        }else if (opt_planner_str == "iw") {
            bool sketches = false;
            if(opt_printing_sketches > 0) {
                sketches = true;
            }
            planner = new IteratedWidth(sim,
                             opt_frameskip,
                             opt_use_minimal_action_set,
                             num_tracked_atoms,
                             opt_screen_features,
                             ...
                             opt_alpha,
                             opt_use_alpha_to_update_reward_for_death,
                             opt_nodes_threshold, opt_max_depth, opt_novelty_subtables,lookahead_depth, sketches);
    ...
    logging::Logger::Info << "planner=" << planner->name() << endl;
    ...
    // cleanup
    delete planner;
    if( &logging::Logger::output_stream() != default_log_file ) {
        static_cast<ofstream*>(&logging::Logger::output_stream())->close();
    }
