...
#SBATCH --output=output16.txt  # Output log file
#SBATCH --error=error16.err
...
apptainer exec \
    ...
    mycontainer2.sif \
      sh -c "cd src && make clean && make && echo 32 128 50000 time 10 000 000 sim max_execution_length 75000 lookahead caching 2 max_rep 60000 max depth 900 000>> output16.txt && python3 running.py /work/rleap1/aaditya_mehta/Sketches--IW1_temporary/Adventure.bin 50000 10000000 --additional-args='--nodes-threshold 200000000 --max-depth 900000 --max-execution-length 75000 --lookahead_depth 10000 --max-rep 60000 --lookahead-caching 2' --compile-first --output-file=output16.txt"
